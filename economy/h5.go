...
func (e *Economy) GetEconomyH5URL(ctx context.Context, dealerUserId, notifyUrl, color, returnUrl, customTitle string, clientType int) (h5url string, err error) {
	var (
		apiName = "预启动接口"
		req     = &reqH5URL{
			DealerId:     e.Dealer,
	...
	responseBytes, err := core.PostJSON(e.newContext(ctx, core.NewRsaSing(e.PrivateKey)), preStartURI, req)
	if err != nil {
		return
	}
...
// Register 工商实名信息录入
func (e *Economy) Collect(ctx context.Context, DealerUserId, PhoneNo, IdCard, RealName, IdCardAddress, IdCardAgency, IdCardNation, IdCardValidityStart, IdCardValidityEnd string) (dealerId string, err error) {
	var (
		apiName = "工商实名信息录入"
		req     = &reqCollect{
			DealerId:            e.Dealer,
	...
	responseBytes, err := core.PostJSON(e.newContext(ctx, core.NewRsaSing(e.PrivateKey)), collectURI, req)
	if err != nil {
		return
	}
...
func (e *Economy) GetEconomyStatusWithOpenId(ctx context.Context, OpenId string) (status EconomyStatus, err error) {
	var (
		apiName = "查询个体工商户状态"
	...
	responseBytes, err := core.PostJSON(e.newContext(ctx, core.NewRsaSing(e.PrivateKey)), economyStatusURI, req)
	if err != nil {
		return status, err
	}
...
func (e *Economy) GetEconomyStatusWithIdCard(ctx context.Context, readName, idCard, dealerUserId string) (status EconomyStatus, err error) {
	var (
		apiName = "查询个体工商户状态"
		req     = &reqEconomyStatus{
			DealerId:     e.Dealer,
	...
	responseBytes, err := core.GetJson(e.newContext(ctx, core.NewRsaSing(e.PrivateKey)), economyStatusURI, req)
	if err != nil {
		return status, err
	}
